...
const ProductModal = ({ product, onClose, onAction, tab }) => {
    ...
    const toAttrs = (v) => {
        if (!v) return {};
        if (v.attributes) {
            if (Array.isArray(v.attributes)) return Object.fromEntries(v.attributes.map(a => [a.key, a.value]));
            return v.attributes;
        }
        const fallback = {};
        if (v.color) fallback.color = v.color;
        if (v.size) fallback.size = v.size;
        return fallback;
    };

    const keyOf = (v) => {
        const entries = Object.entries(toAttrs(v) || {}).sort(([a],[b]) => a.localeCompare(b));
        return JSON.stringify(entries);
    };

    const existingVariantKeys = new Set((product.variants || []).map(keyOf));
    const pendingVariants = (product.pendingChanges?.variants || []).filter(v => !existingVariantKeys.has(keyOf(v)));

    const toggleSelect = (k) => {
        setSelectedPendingKeys(s => s.includes(k) ? s.filter(x => x !== k) : [...s, k]);
    };
        ...
        <>
            <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
                <div className="rounded-lg shadow-xl w-full max-w-3xl transform transition-all" style={{ backgroundColor: 'var(--surface-1)', color: 'var(--component-text)' }}>
                    ...
                    <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6 max-h-[70vh] overflow-y-auto">
                        <div>
                            ...
                            {pendingVariants.length > 0 && (
                                <div className="mt-4 p-4 rounded-lg" style={{ backgroundColor: 'var(--surface-2)', border: '1px solid var(--border-color)' }}>
                                    <h4 className="font-bold mb-2">Proposed Variants (Seller Request)</h4>
                                    <div className="text-xs text-[var(--muted-text)] mb-3">Review and approve or request changes. Images shown as counts.</div>
                                        ...
                                        <table className="w-full text-left text-sm">
                                            ...
                                            <tbody>
                                                {pendingVariants.map((v, idx) => {
                                                    const k = keyOf(v);
                                                    const attrs = toAttrs(v);
                                                    return (
                                                        <tr key={idx} className="border-b" style={{ borderColor: 'var(--border-color)' }}>
                                                            <td className="p-2 align-top">
                                                                <input type="checkbox" checked={selectedPendingKeys.includes(k)} onChange={() => toggleSelect(k)} aria-label="Select variant" />
                                                            </td>
                                                            <td className="p-2 align-top">{Object.entries(attrs).map(([a,b]) => `${a}: ${b}`).join(' • ') || '—'}</td>
                                                            <td className="p-2 align-top">{Number(v.stock ?? 0)}</td>
                                                            <td className="p-2 align-top">{Array.isArray(v.images) ? v.images.length : 0}</td>
                                                        </tr>
                                                    );
                                                })}
                                            </tbody>
                    ...
                    <div className="p-6 flex justify-between items-center border-t flex-wrap gap-3" style={{ backgroundColor: 'var(--surface-2)', borderColor: 'var(--border-color)' }}>
                        ...
                        <div className="flex justify-end space-x-3 ml-auto">
                            {pendingVariants.length > 0 && (
                                <>
                                    <button onClick={() => handleActionClick('modification')} className="text-white font-bold py-2 px-4 rounded" style={{ backgroundColor: 'var(--btn-yellow)' }}>Request Changes</button>
                                    <button
                                        onClick={() => {
                                            const payload = selectedPendingKeys.length > 0 ? { variantKeys: selectedPendingKeys } : { variantKeys: pendingVariants.map(v => keyOf(v)) };
                                            onAction(product.id, 'approve_variants', payload);
                                            onClose();
                                        }}
                                        className="text-white font-bold py-2 px-4 rounded"
                                        style={{ backgroundColor: 'var(--btn-green)' }}
                                    >
                                        {selectedPendingKeys.length > 0 ? 'Approve Selected' : 'Approve All Proposed'}
                                    </button>
                                </>
                            )}
                            {tab === 'flagged' ? (
